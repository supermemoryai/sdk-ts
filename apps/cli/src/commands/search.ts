...
import { parseEnumArg, parseFloatArg, parseIntArg, parseJsonArg } from '../lib/validators.js';
...
export const searchCommand = defineCliCommand({
  ...
  args: {
    ...
    mode: {
      type: 'string',
      description: 'Search mode: memories, hybrid, or documents (default: memories)',
    },
    include: {
      type: 'string',
      description: 'Comma-separated: summaries,documents,relatedMemories,forgottenMemories',
    },
    filter: {
      type: 'string',
  ...
  async handler({ args, flags, config, span }) {
    ...
    body.threshold = parseFloatArg(args.threshold as string | undefined, 'threshold', { min: 0, max: 1 });
    ...
    if (args.include) {
      const validFields = ['summaries', 'documents', 'relatedMemories', 'forgottenMemories'] as const;
    ...
    output(
      ...
      () => {
        ...
        const header = `  ${results.length} result${results.length === 1 ? '' : 's'} ${chalk.dim(
          `(${data.timing ?? timing}ms)`,
        )}`;
        const items = results.map((r) => {
          ...
          if (r.documents?.length) {
            meta.push(`${r.documents.length} doc${r.documents.length > 1 ? 's' : ''}`);
          }
        ...
        const numbered = items.map((item, i) => `  ${chalk.dim(`${i + 1}.`)} ${item}`).join('\n\n');

        const hints: string[] = [];
        const firstWithDoc = results.find((r) => r.documents?.length);
        if (firstWithDoc?.documents?.[0]) {
          hints.push(`${chalk.dim('View source doc:')} supermemory docs get ${firstWithDoc.documents[0].id}`);
        }
        hints.push(
