		...
		output(
			...
			() => {
				...
				const pairs: [string, string][] = [
					["Plan:", chalk.cyan(data.plan ?? "unknown")],
				]
				...
				pairs.push([
					...
					usage.tokens.limit > 0
						? `${usage.tokens.used.toLocaleString()} / ${usage.tokens.limit.toLocaleString()}  ${formatProgressBar(usage.tokens.used, usage.tokens.limit)}`
						: `${usage.tokens.used.toLocaleString()}`,
				])
				pairs.push([
					"Queries:",
					usage.queries.limit > 0
						? `${usage.queries.used.toLocaleString()} / ${usage.queries.limit.toLocaleString()}  ${formatProgressBar(usage.queries.used, usage.queries.limit)}`
		...
		output(
			...
			() => {
				...
				if (data.periodStart && data.periodEnd) {
					lines.push(
						`  Period: ${chalk.dim(`${new Date(data.periodStart).toLocaleDateString()} - ${new Date(data.periodEnd).toLocaleDateString()}`)}`,
					)
					lines.push("")
				}
				...
				const rows = items.map((item) => [
					...
					item.unit,
					item.limit > 0
						? formatProgressBar(item.used, item.limit)
						: chalk.dim("-"),
				])

				lines.push(
					formatTable(["METRIC", "USED", "LIMIT", "UNIT", "USAGE"], rows),
				)
...
export const billingCommand = defineCommand({
	...
	async run({ rawArgs }) {
		...
		process.stderr.write(
			`\n  ${chalk.dim("Available commands: show, usage, invoices. Run supermemory billing --help for details.")}\n`,
		)
	},
})
