...
async function loginWithApiKey(
	...
	try {
		...
		output(
			...
			() => {
				const lines = [
					`${chalk.green("✓")} Logged in as ${chalk.bold(session.user.email)}`,
				]
				if (session.org) {
					lines.push(
						`${chalk.green("✓")} Organization: ${session.org.name} (${session.org.slug})`,
					)
				}
				lines.push(
					`${chalk.green("✓")} Credentials saved to ~/.supermemory/credentials.json`,
				)
				return lines.join("\n")
			},
			flags,
...
export const logoutCommand = defineCommand({
	...
	async run({ args }) {
		...
		output(
			{ loggedOut: true },
			() => `${chalk.green("✓")} Logged out. Credentials removed.`,
			flags,
		)
	},
})
					...
					output(
						...
						() => {
							const lines: [string, string][] = [
								...
								[
									"Auth:",
									`${whoamiData.auth.method} (${whoamiData.auth.keyPrefix}) ${whoamiData.scope.type === "scoped" ? chalk.yellow("[scoped]") : chalk.dim("[full access]")}`,
								],
							]
							...
							if (whoamiData.scope.type === "scoped") {
								lines.push([
									"Scope:",
									`scoped to ${whoamiData.scope.tag} (enforced)`,
								])
								if (whoamiData.scope.permission)
									lines.push(["Permission:", whoamiData.scope.permission])
								if (whoamiData.scope.rateLimit)
									lines.push([
										"Rate Limit:",
										`${whoamiData.scope.rateLimit} req/min`,
									])
								if (whoamiData.scope.expires)
									lines.push(["Expires:", whoamiData.scope.expires])
							}
