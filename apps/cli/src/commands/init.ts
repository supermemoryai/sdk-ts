...
export const initCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    const hasFlags =
      args.scope || args.tag || args['api-key'] || args.output || args['api-url'] || args['console-url'];
...
async function runInteractive(flags: { json?: boolean }) {
  ...
  process.stderr.write(
    `    API Key:  ${
      auth.apiKey.masked ?
        `${auth.apiKey.masked}  ${chalk.dim(`(${auth.apiKey.source})`)}`
      : chalk.dim('(not set)')
    }\n`,
  );
  process.stderr.write(
    `    Tag:      ${
      auth.tag.value ? `${auth.tag.value}  ${chalk.dim(`(${auth.tag.source})`)}` : chalk.dim('(not set)')
    }\n`,
  );
  process.stderr.write('\n');
...
async function runProjectWizard(
  ...
  if (apiKeyAnswer && apiKeyAnswer !== auth.apiKey.masked) {
    try {
      ...
      exitWithError('invalid_api_key', 'API key validation failed. Check your key and try again.', flags);
    }
  }
  ...
  if (tagAnswer && tagAnswer !== (existing?.tag ?? '')) projectUpdates.tag = tagAnswer;
...
async function runTeamWizard(auth: ReturnType<typeof getAuthState>, flags: { json?: boolean }) {
  const gitRoot = findGitRoot();
  if (!gitRoot) {
    exitWithError('not_git_repo', 'Not in a git repository. Team config requires a git repo.', flags);
  }
  ...
  output(
    ...
    () => {
      ...
      lines.push(`  ${chalk.dim('Commit .supermemory/ to share config with your team.')}`);
      return lines.join('\n');
    },
    flags,
  );
...
async function runGlobalWizard(config: ReturnType<typeof resolveConfig>, flags: { json?: boolean }) {
  ...
  if (outputAnswer && ['auto', 'json', 'human'].includes(outputAnswer)) updates.output = outputAnswer;
...
async function runNonInteractive(args: Record<string, unknown>, flags: { json?: boolean }) {
  ...
  if ((args.output || args['api-url'] || args['console-url']) && scope !== 'global') {
    exitWithError(
      'invalid_flag',
  ...
  if (scope === 'project') {
    ...
    const updates: Record<string, string> = {};
    if (args.output) {
      parseEnumArg(args.output as string, 'output', ['auto', 'json', 'human'] as const);
    ...
    output(
      ...
      () => {
        ...
        if (updates.consoleUrl) lines.push(`  Console URL: ${updates.consoleUrl}`);
        lines.push(`  Saved to ${chalk.dim('~/.supermemory/config.json')}`);
        return lines.join('\n');
      },
      flags,
