		...
		if (!args.browser) {
			...
			if (!shouldOutputJson(flags)) {
				process.stderr.write(
					`  Opened browser for ${chalk.bold(provider)} authorization...\n`,
				)
			}
		}
			...
			if (repos.length === 0) {
				...
				const selectedNames = await multiSelect({
					...
					items: repos.map((r) => ({
						...
						label: r.full_name,
						hint:
							[r.description, r.private ? "(private)" : undefined]
								.filter(Boolean)
								.join(" ") || undefined,
					})),
				})
				...
				const selectedRepos = repos.filter((r) =>
					selectedNames.includes(r.full_name),
				)
				...
				if (selectedRepos.length > 0) {
					...
					success(
						`Configured ${selectedRepos.length} repositories${configureResult.webhooksRegistered != null ? ` with ${configureResult.webhooksRegistered} webhooks` : ""}`,
						flags,
					)
				}
			}
...
const connectorsSyncCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		const { data: connections } = await apiRequest<
			{ id: string; provider: string }[]
		>("/v3/connections/list", { method: "POST", body: {} })
	...
	async handler({ args, flags }) {
		if (!args.yes) {
			const confirmed = await confirm(
				`${chalk.yellow("⚠")} Disconnect connection ${chalk.bold(args.id as string)}? This cannot be undone. [y/N] `,
			)
			if (!confirmed) {
		...
		const { data } = await apiRequest<{ id: string; provider: string }>(
			`/v3/connections/${args.id}`,
			{ method: "DELETE" },
		)
...
export const connectorsCommand = defineCommand({
	...
	async run({ rawArgs }) {
		...
		process.stderr.write(
			`\n  ${chalk.dim("Available commands: list, connect, sync, history, disconnect, resources. Run supermemory connectors --help for details.")}\n`,
		)
	},
})
