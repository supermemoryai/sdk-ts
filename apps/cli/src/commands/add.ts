...
async function handleTextAdd(
	...
	output(
		...
		() => {
			const lines = [
				`${chalk.green("✓")} Added document ${chalk.dim(data.id)} (${data.status ?? "queued"})`,
			]
...
async function handleUrlAdd(
	...
	output(
		...
		() => {
			const lines = [
				`${chalk.green("✓")} Added URL ${chalk.dim(url)} → ${chalk.dim(data.id)} (${data.status ?? "queued"})`,
			]
			if (data.containerTag || tag) {
				lines.push(`  Tag: ${data.containerTag ?? tag}`)
			}
...
async function handleFileAdd(
	...
	output(
		...
		() => {
			const lines = [
				`${chalk.green("✓")} Uploaded ${chalk.dim(fileName)} → ${chalk.dim(data.id)} (${data.status ?? "queued"})`,
			]
			if (data.containerTag || tag) {
				lines.push(`  Tag: ${data.containerTag ?? tag}`)
			}
...
async function handleBatch(
	...
	try {
		...
		return exitWithError(
			"validation_error",
			"Invalid JSON array from stdin for batch mode",
			flags,
		)
	}
