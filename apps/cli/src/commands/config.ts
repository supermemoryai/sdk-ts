	...
	async handler({ args, flags }) {
		...
		if (!scope) {
			throw new ValidationError(
				`Invalid scope. Valid scopes: ${VALID_SCOPES.join(", ")}`,
			)
		}
		...
		if (!matrix) {
			throw new ValidationError(
				`Unknown config key "${key}". Valid keys: ${VALID_SET_KEYS.join(", ")}`,
			)
		}
		...
		const fieldName =
			scope === "global" && matrix.globalField ? matrix.globalField : key
		...
		output(
			{ key, value, scope },
			() => `${chalk.green("✓")} Set ${key} = ${value} (${scope} scope)`,
			flags,
		)
	},
})

const VALID_RESOLVED_KEYS = [
	"apiKey",
	"tag",
	"apiUrl",
	"consoleUrl",
...
const configGetCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		...
		const value = (resolved as unknown as Record<string, unknown>)[
			args.key as string
		]

		const displayValue =
			args.key === "apiKey" && typeof value === "string"
				? maskApiKey(value)
				: String(value ?? "(none)")
...
export const configCommand = defineCommand({
	...
	async run({ rawArgs, args }) {
		...
		output(
			...
			() => {
				const lines = [
					`  ${chalk.bold("Resolved Configuration")} (highest precedence wins)`,
					"",
				]

				const keyScope = getKeyScope()
				const scopeLabel =
					keyScope?.type === "scoped"
						? chalk.yellow("[scoped]")
				...
				const pairs: [string, string][] = [
					[
						"API Key:",
						resolved.apiKey
							? `${maskApiKey(resolved.apiKey)} ${scopeLabel}`
							: chalk.dim("(none)"),
					],
				...
				lines.push(
					`  ${chalk.bold("User Scope")} ${chalk.dim("(~/.supermemory/)")}`,
				)
				...
				lines.push(
					`    Credentials: ${credentials ? chalk.green("stored") : chalk.dim("none")}`,
				)
				lines.push("")

				lines.push(
					`  ${chalk.bold("Team Scope")} ${chalk.dim("(.supermemory/ in repo)")}`,
				)
				if (teamConfig) {
					...
					lines.push(
						`    ${chalk.dim("(not found — run supermemory init to create)")}`,
					)
				}
				lines.push("")

				lines.push(
					`  ${chalk.bold("Project Scope")} ${chalk.dim("(~/.supermemory/projects/-path-/)")}`,
				)
				if (projectConfig) {
