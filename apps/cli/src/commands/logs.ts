...
function formatStatusCode(code: number): string {
	return code >= 200 && code < 300
		? chalk.green(String(code))
		: chalk.red(String(code))
}
...
const logsListCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		...
		query.status = parseEnumArg(args.status as string | undefined, "status", [
			"success",
			"error",
		] as const)
		query.limit =
			parseIntArg(args.limit as string | undefined, "limit", { min: 1 }) ?? 20
		query.page =
			parseIntArg(args.page as string | undefined, "page", { min: 1 }) ?? 1
		...
		output(
			...
			() => {
				...
				const header = pg
					? `  Page ${pg.currentPage} of ${pg.totalPages} (${pg.totalItems} total requests)\n`
					: ""

				const rows = logs.map((r) => {
					const preview = r.input
						? truncate(JSON.stringify(r.input), 60)
						: chalk.dim("--")
					return [
				...
				const table = formatTable(
					["TIMESTAMP", "TYPE", "STATUS", "LATENCY", "CONTENT", "ID"],
					rows,
				)
		...
		output(
			...
			() => {
				...
				const pairs: [string, string][] = [
					...
					[
						"Latency:",
						log.duration != null ? `${log.duration}ms` : chalk.dim("--"),
					],
				]
				...
				if (log.input) {
					...
					lines.push(
						`  ${chalk.dim(JSON.stringify(log.input, null, 2).replace(/\n/g, "\n  "))}`,
					)
				}
				...
				if (log.output) {
					...
					lines.push(
						`  ${chalk.dim(JSON.stringify(log.output, null, 2).replace(/\n/g, "\n  "))}`,
					)
				}
