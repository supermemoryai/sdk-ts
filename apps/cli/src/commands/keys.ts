			...
			() => {
				const keys = (data.keys ?? []).filter((k) => {
					...
					try {
						...
						return (
							meta.sm_type !== "agent_cli" && meta.sm_type !== "plugin_auth"
						)
					} catch {
						return true
					}
				...
				return formatTable(
					...
					keys.map((k) => [
						...
						k.name,
						k.containerTags?.length
							? `scoped (${k.containerTags.join(", ")})`
							: k.containerTag
								? `scoped (${k.containerTag})`
								: "full",
						k.permission,
...
const keysCreateCommand = defineCliCommand({
	...
	args: {
		...
		tag: {
			type: "string",
			description:
				"Scope key to container tag(s), comma-separated for multiple",
		},
		expires: {
			type: "string",
	...
	async handler({ args, flags }) {
		const permission =
			parseEnumArg(args.permission as string | undefined, "permission", [
				"read",
				"write",
			] as const) ?? "write"
		...
		output(
			...
			() => {
				...
				lines.push(
					`${chalk.green("✓")} Created API key ${chalk.bold(data.name)}`,
				)
				lines.push("")
				lines.push(
					`  ${chalk.yellow("WARNING:")} This key will only be shown once. Copy it now.`,
				)
				lines.push("")
				...
				lines.push(
					formatKeyValue([
						...
						[
							"Scope:",
							data.containerTags?.length
								? `scoped (${data.containerTags.join(", ")})`
								: data.containerTag
									? `scoped (${data.containerTag})`
									: "full",
						],
						[
							"Expires:",
	...
	async handler({ args, flags }) {
		if (!args.yes) {
			const confirmed = await confirm(
				`${chalk.yellow("⚠")} Revoke API key ${chalk.bold(args.id as string)}? This cannot be undone. [y/N] `,
			)
			if (!confirmed) {
		...
		output(
			data,
			() =>
				`${chalk.green("✓")} Revoked API key ${chalk.dim(args.id as string)}`,
			flags,
		)
	},
})
...
const keysToggleCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		...
		output(
			data,
			() =>
				`${chalk.green("✓")} Key ${chalk.dim(args.id as string)} is now ${data.enabled ? chalk.green("enabled") : chalk.red("disabled")}`,
			flags,
		)
	},
})
...
export const keysCommand = defineCommand({
	...
	async run({ rawArgs }) {
		...
		process.stderr.write(
			`\n  ${chalk.dim("Available commands: list, create, revoke, toggle. Run supermemory keys --help for details.")}\n`,
		)
	},
})
