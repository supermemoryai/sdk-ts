	...
	async handler({ args, flags }) {
		...
		output(
			...
			() => {
				...
				lines.push(
					`${chalk.green("✓")} Connected plugin ${chalk.bold(data.pluginId)}`,
				)
				lines.push("")
				lines.push(
				...
				if (args["auto-configure"]) {
					...
					if (configPath) {
						...
						lines.push(
							`  Exists:      ${existsSync(configPath) ? chalk.green("yes") : chalk.dim("no")}`,
						)
						lines.push("")
						lines.push(
							`  Add the following key to your ${chalk.bold(args.id as string)} configuration:`,
						)
						lines.push(`  ${chalk.bold(data.key)}`)
					}
				}
...
const pluginsRevokeCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		...
		if (!pluginKey) {
			exitWithError(
				"not_found",
				`No connected plugin found for "${pluginName}"`,
				flags,
			)
		}
		...
		output(
			data,
			() =>
				`${chalk.green("✓")} Revoked API key for plugin ${chalk.dim(pluginName)}`,
			flags,
		)
	},
})
					...
					formatKeyValue([
						...
						[
							...
							PLUGIN_CONFIG_PATHS[p.pluginId]
								? chalk.cyan(PLUGIN_CONFIG_PATHS[p.pluginId])
								: chalk.dim("--"),
						],
						[
							"Config Exists:",
							PLUGIN_CONFIG_PATHS[p.pluginId]
								? existsSync(PLUGIN_CONFIG_PATHS[p.pluginId] as string)
									? chalk.green("yes")
									: chalk.dim("no")
								: chalk.dim("--"),
						],
...
export const pluginsCommand = defineCommand({
	...
	async run({ rawArgs }) {
		...
		process.stderr.write(
			`\n  ${chalk.dim("Available commands: list, connect, revoke, status. Run supermemory plugins --help for details.")}\n`,
		)
	},
})
