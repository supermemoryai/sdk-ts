...
import { EXIT, exitWithError, formatKeyValue, formatTable, output } from '../lib/output.js';
  ...
  async handler({ flags }) {
    ...
    output(
      ...
      () => {
        ...
        if (tags.length === 0) {
          const hints = [`${chalk.dim('Create tag:')} supermemory tags create <name>`];
        ...
        const table = formatTable(
          ...
          tags.map((t) => [
            ...
            t.lastActivityAt ? new Date(t.lastActivityAt).toLocaleDateString() : chalk.dim('never'),
          ]),
        );
    ...
    output(
      ...
      () =>
        formatKeyValue([
          ...
          ['Created:', data.createdAt ? new Date(data.createdAt).toLocaleDateString() : chalk.dim('unknown')],
          [
            'Last Activity:',
            data.lastActivityAt ? new Date(data.lastActivityAt).toLocaleDateString() : chalk.dim('never'),
          ],
...
const tagsCreateCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    ...
    output(
      data,
      () =>
        `${chalk.green('✓')} Created container tag ${chalk.bold(data.containerTag)}${
          data.name ? ` (${data.name})` : ''
        }`,
      flags,
    );
  ...
  async handler({ args, flags }) {
    if (!args.yes) {
      const confirmed = await confirm(
        `${chalk.yellow('⚠')} Delete container tag ${chalk.bold(
          args.tag as string,
    ...
    output(data, () => `${chalk.green('✓')} Deleted container tag ${chalk.bold(args.tag as string)}`, flags);
  },
});
...
const tagsContextCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    ...
    if (args.clear) {
      ...
      output(data, () => `${chalk.green('✓')} Cleared context for ${chalk.bold(args.tag as string)}`, flags);
    } else if (args.set) {
      const { data } = await apiRequest<{
        containerTag: string;
      ...
      output(data, () => `${chalk.green('✓')} Updated context for ${chalk.bold(args.tag as string)}`, flags);
    } else {
      const { data } = await apiRequest<{
        containerTag: string;
...
const tagsMergeCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    ...
    output(
      data,
      () =>
        `${chalk.green('✓')} Merged ${chalk.bold(args.source as string)} into ${chalk.bold(
          args.into as string,
        )}${data.mergedCount != null ? ` (${data.mergedCount} items)` : ''}`,
      flags,
    );
  },
});
...
export const tagsCommand = defineCommand({
  ...
  async run({ rawArgs }) {
    ...
    if (sub && ['list', 'info', 'create', 'delete', 'context', 'merge'].includes(sub)) return;
    await runCommand(tagsListCommand, { rawArgs });
    process.stderr.write(
      `\n  ${chalk.dim(
        'Available commands: list, info, create, delete, context, merge. Run supermemory tags --help for details.',
      )}\n`,
    );
  },
});
