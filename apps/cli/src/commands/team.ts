...
const teamInviteCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		const role =
			parseEnumArg(args.role as string | undefined, "role", [
				"admin",
				"member",
			] as const) ?? "member"
...
const teamRemoveCommand = defineCliCommand({
	...
	async handler({ args, flags }) {
		...
		output(
			data,
			() => `${chalk.green("✓")} Removed team member ${chalk.dim(memberId)}`,
			flags,
		)
	},
})
...
export const teamCommand = defineCommand({
	...
	async run({ rawArgs }) {
		...
		if (
			sub &&
			["list", "invite", "remove", "role", "invitations"].includes(sub)
		)
			return
		await runCommand(teamListCommand, { rawArgs })
		process.stderr.write(
			`\n  ${chalk.dim("Available commands: list, invite, remove, role, invitations. Run supermemory team --help for details.")}\n`,
		)
	},
})
