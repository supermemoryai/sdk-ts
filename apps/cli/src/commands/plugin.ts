...
function writePluginCredentials(id: PluginId, apiKey: string): void {
	...
	const payload =
		id === "cursor" || id === "opencode" ? createdAtPayload : savedAtPayload
	...
	return new Promise((resolveServer, rejectServer) => {
		...
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (url.searchParams.get("state") !== stateToken) {
				...
				res.end(
					callbackHtml("Authentication Failed", "Invalid callback state."),
				)
				clearAuthTimeout()
			...
			try {
				...
				const errorsByClient: Partial<Record<PluginClientId, string>> =
					errorsPayload
						? (decodeBase64UrlJson(errorsPayload) as Partial<
								Record<PluginClientId, string>
							>)
						: {}
				...
				rejectCallback(
					error instanceof Error
						? error
						: new Error("Invalid OAuth callback payload"),
				)
			}
		})
...
async function authorizeInstalledPlugins(
	...
	const installed = results.filter(
		(result): result is InstallResult & { status: "installed" } =>
...
function printHumanSummary(
	...
	if (authTargets.length > 0 && noAuth) {
		process.stdout.write(
			`  ${chalk.dim("Auth:")} skipped because --no-auth was used.\n\n`,
		)
	}
