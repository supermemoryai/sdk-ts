...
const PIPELINE_STAGES = ['queued', 'extracting', 'chunking', 'embedding', 'indexing', 'done'] as const;
...
const docsListCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    ...
    output(
      data,
      () => {
        const tagLabel = `  ${chalk.dim('Container tag:')} ${chalk.bold(tag)} ${chalk.dim(
          `(from ${tagSource})`,
        )}`;
        const results = data.documents ?? [];
        if (results.length === 0) {
          const hints = [`${chalk.dim('Add content:')} supermemory add <url-or-text> --tag ${tag}`];
          return `${tagLabel}\n\n  No documents found.\n\n${hints.map((h) => `  ${h}`).join('\n')}`;
        }
        ...
        const table = formatTable(
          ...
          results.map((doc) => {
            return [doc.id, truncate(doc.title ?? '', 40), doc.status, formatRelativeTime(doc.createdAt)];
          }),
        );
        ...
        if (pagination && pagination.currentPage < pagination.totalPages) {
          hints.push(
            `${chalk.dim('Next page:')} supermemory docs list --tag ${tag} --page ${
              pagination.currentPage + 1
            }`,
          );
        }
        const processing = results.find((d) => d.status !== 'done' && d.status !== 'failed');
        if (processing) {
          hints.push(`${chalk.dim('Check pipeline:')} supermemory docs status ${processing.id}`);
        }
    ...
    output(
      ...
      () => {
        ...
        return formatKeyValue([
          ...
          ['Updated:', data.updatedAt ? formatRelativeTime(data.updatedAt) : ''],
        ]);
      },
      flags,
    );
...
const docsDeleteCommand = defineCliCommand({
  ...
  async handler({ args, flags, span }) {
    ...
    output(data, () => `${chalk.green('\u2713')} Deleted document ${chalk.dim(args.id as string)}`, flags);
  },
});
...
const docsStatusCommand = defineCliCommand({
  ...
  async handler({ args, flags }) {
    ...
    const progress =
      isFailed ? 0
      : currentIndex >= 0 ? Math.round(((currentIndex + 1) / PIPELINE_STAGES.length) * 100)
      : 0;
    ...
    output(
      ...
      () => {
        if (isFailed) {
          const lines = PIPELINE_STAGES.map((stage) => `  ${chalk.dim('\u25CB')} ${chalk.dim(stage)}`);
...
export const docsCommand = defineCommand({
  ...
  async run({ rawArgs }) {
    ...
    if (sub && ['list', 'get', 'delete', 'chunks', 'status'].includes(sub)) return;
    await runCommand(docsListCommand, { rawArgs });
    process.stderr.write(
      `\n  ${chalk.dim(
        'Available commands: list, get, delete, chunks, status. Run supermemory docs --help for details.',
      )}\n`,
    );
  },
});
