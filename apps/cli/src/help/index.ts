...
interface CommandSchema {
  ...
  options?: Record<string, { type: string; default?: unknown; description: string }>;
  examples?: string[];
}
...
const FULL_COMMANDS: Record<string, CommandSchema> = {
  ...
  billing: {
    ...
    examples: ['supermemory billing', 'supermemory billing usage', 'supermemory billing invoices'],
  },
  plugins: {
  ...
  connectors: {
    ...
    usage: 'supermemory connectors <list|connect|sync|history|disconnect|resources>',
    options: {
  ...
  open: {
    ...
    arguments: {
      page: {
        required: false,
        description: 'Page: console, billing, graph, connectors, settings, docs, keys, team, agents',
      },
    },
    options: {
      '--no-browser': {
    ...
    examples: ['supermemory open', 'supermemory open graph', 'supermemory open billing'],
  },
  team: {
  ...
  init: {
    description: 'Initialize Supermemory configuration (interactive wizard or flags)',
    usage: 'supermemory init [--scope <project|team|global>] [options]',
    options: {
      '--scope': {
        type: 'string',
        description: 'Config scope: project (local machine), team (committed to git), global (all projects)',
      },
      '--tag': { type: 'string', description: 'Container tag' },
      '--api-key': {
        type: 'string',
  ...
  plugin: {
    ...
    options: {
      ...
      '--force': {
        type: 'boolean',
        default: false,
        description: 'Run selected installers even if the target app is not detected',
      },
      '--no-auth': {
        type: 'boolean',
        default: false,
...
export function formatHelpText(
  ...
  const lines = ['', `${chalk.bold('supermemory')} — memory layer for AI agents`, ''];
  ...
  lines.push('');
  lines.push(`  ${chalk.dim('supermemory <command> --help')}  Get help for a command`);
  lines.push(`  ${chalk.dim('supermemory help --json')}      Machine-readable help (for LLM agents)`);
  ...
  if (authState) {
    ...
    lines.push(
      `    API Key   ${authState.apiKey.masked ?? chalk.dim('—')}   ${
        authState.apiKey.source ? chalk.dim(`(${authState.apiKey.source})`) : chalk.dim('(not set)')
      }  ${mode === 'scoped' ? chalk.yellow('[scoped]') : chalk.dim('[full access]')}`,
    );
    if (mode === 'scoped' && options?.scope?.tag) {
      const scopeDisplay =
        ...
        : `"${options.scope.tag}"`;
      lines.push(
        `    Container Tag   Scoped to ${scopeDisplay}   ${chalk.dim(
          `(${options.scope.permission ?? 'write'})`,
        )}`,
      );
    } else {
      lines.push(
        `    Container Tag   ${authState.tag.value ?? chalk.dim('—')}   ${
          authState.tag.source ? chalk.dim(`(${authState.tag.source})`) : chalk.dim('(not set)')
        }`,
      );
    }
  }
