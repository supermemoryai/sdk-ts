...
async function detectScope(): Promise<{
	...
	try {
		...
		const tags =
			session.scope.tags ??
			(session.scope.tag ? [session.scope.tag] : undefined)
...
async function main() {
			...
			if (args.json) {
				...
				process.stdout.write(
					formatHelpText(mode, authState, {
						all: true,
						scope: scope
							? { tag: scope.tag, permission: scope.permission }
							: undefined,
					}),
				)
			}
	...
	if (rawArgs.includes("--help") && rawArgs.includes("--json")) {
		const commandName = rawArgs.find(
			(a) => !a.startsWith("--") && a in subCommands,
		)
		if (commandName) {
			const schema = getCommandSchema(commandName)
			if (schema) {
				process.stdout.write(
					`${JSON.stringify({ name: commandName, ...schema }, null, 2)}\n`,
				)
				await shutdownOtel()
				return
			}
		...
		if (resolved) {
			...
			process.stdout.write(
				formatHelpText(mode, authState, {
					all: showAll,
					scope: scope
						? { tag: scope.tag, tags: scope.tags, permission: scope.permission }
						: undefined,
				}),
			)
		}
