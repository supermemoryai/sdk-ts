...
function detectPackageManager(cwd: string, pyprojectContent?: string | null): string | null {
  if (existsSync(join(cwd, 'bun.lockb')) || existsSync(join(cwd, 'bun.lock'))) return 'bun';
  if (existsSync(join(cwd, 'pnpm-lock.yaml'))) return 'pnpm';
  if (existsSync(join(cwd, 'yarn.lock'))) return 'yarn';
  if (existsSync(join(cwd, 'package-lock.json'))) return 'npm';

  const pyproject = pyprojectContent ?? readFileSafe(join(cwd, 'pyproject.toml'));
...
function detectNodeProject(cwd: string): Omit<ProjectDetection, 'hasApiKey'> {
  ...
  const language =
    existsSync(join(cwd, 'tsconfig.json')) || allDeps.has('typescript') ? 'typescript' : 'javascript';
  ...
  const hasSupermemory = allDeps.has('supermemory') || allDeps.has('@supermemory/tools');
  ...
  for (const file of ['.env', '.env.local']) {
    ...
    if (content) {
      const hasKey = content
        .split('\n')
        .some(
          (line) => line.startsWith(API_KEY_PREFIX) && line.slice(API_KEY_PREFIX.length).trim().length > 0,
        );
