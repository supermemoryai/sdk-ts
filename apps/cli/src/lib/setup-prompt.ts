...
export function formatDetection(detection: ProjectDetection): string {
	...
	if (detection.framework)
		parts.push(FRAMEWORK_NAMES[detection.framework] ?? detection.framework)
	if (detection.aiSdk)
		parts.push(AI_SDK_NAMES[detection.aiSdk] ?? detection.aiSdk)
...
function getDocsUrl(aiSdk: string | null): string {
	return (
		SDK_DOCS_MAP[aiSdk ?? ""] ?? `${DOCS_BASE}/integrations/supermemory-sdk`
	)
}
...
function getInstallInfo(detection: ProjectDetection): InstallInfo | null {
	...
	const useTools = ["vercel-ai", "openai", "anthropic", "mastra"].includes(
		aiSdk ?? "",
	)
...
export function generateSetupPrompt(options: {
	...
	const sdkName =
		AI_SDK_NAMES[detection.aiSdk ?? ""] ??
		(isPython ? "Python SDK" : "TypeScript SDK")
	...
	lines.push("## Project")
	lines.push(
		`- Language: ${LANGUAGE_NAMES[detection.language] ?? detection.language}`,
	)
	if (detection.framework)
		lines.push(
			`- Framework: ${FRAMEWORK_NAMES[detection.framework] ?? detection.framework}`,
		)
	if (detection.aiSdk) lines.push(`- AI SDK: ${sdkName}`)
	if (detection.packageManager)
		lines.push(`- Package Manager: ${detection.packageManager}`)
	lines.push(
		`- Supermemory: ${detection.hasSupermemory ? "already installed" : "not installed"}`,
	)
	lines.push("")
	...
	if (!detection.hasSupermemory && installInfo) {
		...
		lines.push("## Install")
		lines.push(
			`No Go SDK available yet. Use the REST API directly: ${DOCS_BASE}/api-reference`,
		)
		lines.push("")
	}
	...
	if (!detection.hasApiKey) {
		...
		lines.push(
			"Add SUPERMEMORY_API_KEY to .env — get one at https://console.supermemory.ai",
		)
		lines.push("")
	}
	...
	lines.push(
		`Other integrations available if needed: ${otherIntegrations.join(", ")}.`,
	)
	lines.push(
		`If the user needs a different integration, refer to: ${DOCS_BASE}/integrations`,
	)
	lines.push("")
	...
	if (isPython) {
		lines.push(
			'Use container tags to scope memory per user: container_tag=f"user_{user_id}"',
		)
	} else {
		lines.push(
	...
	if (config.tag) {
		lines.push(
			`Configured tag: "${config.tag}" — use this if scoping to this project.`,
		)
	}
	lines.push("")
