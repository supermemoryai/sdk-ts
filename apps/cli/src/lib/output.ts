...
export function error(
	...
	if (shouldOutputJson(flags)) {
		process.stderr.write(
			`${JSON.stringify({ error: code, message, ...(hint ? { hint } : {}) })}\n`,
		)
	} else {
...
export function formatTable(
	...
	const cellLengths = rows.map((row) =>
		row.map((cell) => visualLength(cell ?? "")),
	)

	const widths =
		columnWidths ??
		headers.map((h, i) =>
			Math.max(h.length, ...cellLengths.map((lens) => lens[i] ?? 0)),
		)
...
export function formatKeyValue(pairs: [string, string][]): string {
	...
	return pairs
		.map(([key, value]) => `  ${chalk.dim(key.padEnd(maxKeyLen + 1))} ${value}`)
		.join("\n")
}
...
export function formatNumberedList(
	...
	return items
		.map(
			(item, i) =>
				`  ${chalk.dim(`${i + 1}.`)} ${item.title}${item.subtitle ? `\n     ${chalk.dim(item.subtitle)}` : ""}`,
		)
		.join("\n\n")
}
