...
export function readScopeCache(keyHash: string): ScopeCache | null {
	...
	const cache = readJsonFile<ScopeCache>(
		join(SCOPE_CACHE_DIR, `${keyHash}.json`),
	)
...
export function resolveConfig(flags?: { tag?: string }): ResolvedConfig {
	...
	const consoleUrl =
		process.env.SUPERMEMORY_CONSOLE_URL ??
		userConfig.consoleUrl ??
