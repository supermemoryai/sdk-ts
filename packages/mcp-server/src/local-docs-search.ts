...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    ...
    params: [
      'containerTag: string;',
      'buckets?: string[];',
      "filters?: { OR: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; } | { AND: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; };",
      "include?: 'static' | 'dynamic' | 'buckets'[];",
      'q?: string;',
      'threshold?: number;',
    ],
    response:
      '{ profile: { buckets?: object; dynamic?: string[]; static?: string[]; }; searchResults?: { results: { id: string; metadata: object; similarity: number; updatedAt: string; chunk?: string; chunks?: object[]; context?: object; documents?: object[]; filepath?: string; isAggregated?: boolean; memory?: string; rootMemoryId?: string; version?: number; }[]; timing: number; total: number; }; }',
    markdown:
      "## profile\n\n`client.profile(containerTag: string, buckets?: string[], filters?: { OR: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; } | { AND: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; }, include?: 'static' | 'dynamic' | 'buckets'[], q?: string, threshold?: number): { profile: object; searchResults?: object; }`\n\n**post** `/v4/profile`\n\nGet user profile with optional search results\n\n### Parameters\n\n- `containerTag: string`\n  Tag to filter the profile by. This can be an ID for your user, a project ID, or any other identifier you wish to use to filter memories.\n\n- `buckets?: string[]`\n  Specific bucket keys to return. Omit to return all configured buckets. Only relevant when \"buckets\" is included.\n\n- `filters?: { OR: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; } | { AND: { key: string; value: string; filterType?: 'metadata' | 'numeric' | 'array_contains' | 'string_contains'; ignoreCase?: boolean | 'true' | 'false'; negate?: boolean | 'true' | 'false'; numericOperator?: '>' | '<' | '>=' | '<=' | '='; } | { OR: object[]; } | { AND: object[]; }[]; }`\n  Optional metadata filters to apply to profile results and search results. Supports complex AND/OR queries with multiple conditions.\n\n- `include?: 'static' | 'dynamic' | 'buckets'[]`\n  Profile sections to return. Omit to return all sections. Pass a subset to reduce payload — e.g. [\"buckets\"] skips static and dynamic entirely.\n\n- `q?: string`\n  Optional search query to include search results in the response\n\n- `threshold?: number`\n  Threshold for search results. Only results with a score above this threshold will be included.\n\n### Returns\n\n- `{ profile: { buckets?: object; dynamic?: string[]; static?: string[]; }; searchResults?: { results: { id: string; metadata: object; similarity: number; updatedAt: string; chunk?: string; chunks?: object[]; context?: object; documents?: object[]; filepath?: string; isAggregated?: boolean; memory?: string; rootMemoryId?: string; version?: number; }[]; timing: number; total: number; }; }`\n\n  - `profile: { buckets?: object; dynamic?: string[]; static?: string[]; }`\n  - `searchResults?: { results: { id: string; metadata: object; similarity: number; updatedAt: string; chunk?: string; chunks?: { content: string; documentId: string; position: number; }[]; context?: { children?: { memory: string; relation: 'updates' | 'extends' | 'derives'; updatedAt: string; metadata?: object; version?: number; }[]; parents?: { memory: string; relation: 'updates' | 'extends' | 'derives'; updatedAt: string; metadata?: object; version?: number; }[]; related?: { memory: string; relation: 'extends' | 'derives'; updatedAt: string; metadata?: object; }[]; }; documents?: { id: string; createdAt: string; updatedAt: string; metadata?: object; summary?: string; title?: string; type?: string; }[]; filepath?: string; isAggregated?: boolean; memory?: string; rootMemoryId?: string; version?: number; }[]; timing: number; total: number; }`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.profile({ containerTag: 'containerTag' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response: '{ id: string; status: string; }',
    markdown:
      "## add\n\n`client.documents.add(content: string, containerTag?: string, containerTags?: string[], customId?: string, documentDate?: string, dreaming?: 'instant' | 'dynamic', entityContext?: string, filepath?: string, filterByMetadata?: object, metadata?: object, taskType?: 'memory' | 'superrag'): { id: string; status: string; }`\n\n**post** `/v3/documents`\n\nAdd a document with any content type (text, url, file, etc.) and metadata\n\n### Parameters\n\n- `content: string`\n  The content to extract and process into a document. This can be a URL to a website, a PDF, an image, or a video.\n\n- `containerTag?: string`\n  Optional tag this document should be containerized by. Max 100 characters, alphanumeric with hyphens, underscores, and dots only.\n\n- `containerTags?: string[]`\n\n- `customId?: string`\n  Optional custom ID of the document. Max 100 characters, alphanumeric with hyphens, underscores, and dots only.\n\n- `documentDate?: string`\n  When this document's content is from, as opposed to when it was uploaded. Accepts YYYY-MM-DD or a full ISO 8601 timestamp. Memory extraction resolves relative dates against this instead of the ingestion time, and documents in a batch are processed oldest-first so newer facts correctly supersede older ones. Set this whenever you backfill historical content.\n\n- `dreaming?: 'instant' | 'dynamic'`\n  Processing mode. \"dynamic\" (default) groups related documents together so memories form from coherent, logical units rather than one isolated entry at a time. \"instant\" processes each document on its own right away, and bills one extra operation per document.\n\n- `entityContext?: string`\n  Optional entity context for this container tag. Max 1500 characters. Used during document processing to guide memory extraction.\n\n- `filepath?: string`\n  Optional file path for the document. Used by supermemoryfs to store the full path of the file.\n\n- `filterByMetadata?: object`\n  Optional metadata filter to apply when pulling related memories and profile during ingestion. Only memories matching these filters will be used as context.\n\n- `metadata?: object`\n  Optional metadata for the document.\n\n- `taskType?: 'memory' | 'superrag'`\n  Task type: \"memory\" (default) for full context layer with SuperRAG built in, \"superrag\" for managed RAG as a service.\n\n### Returns\n\n- `{ id: string; status: string; }`\n\n  - `id: string`\n  - `status: string`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.documents.add({ content: 'content' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: ['id: string;'],
    response:
      "{ id: string; connectionId: string; content: string; createdAt: string; customId: string; dreamingStatus: 'dreaming' | 'done'; filepath: string; metadata: string | number | boolean | object | object[]; ogImage: string; raw: object; source: string; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; taskType: 'memory' | 'superrag'; title: string; type: string; updatedAt: string; containerTags?: string[]; memories?: { id: string; createdAt: string; isForgotten: boolean; isInference: boolean; isLatest: boolean; isStatic: boolean; memory: string; metadata: string | number | boolean | object | object[]; parentMemoryId: string; rootMemoryId: string; sourceCount: number; updatedAt: string; version: number; }[]; url?: string; }",
    markdown:
      "## get\n\n`client.documents.get(id: string): { id: string; connectionId: string; content: string; createdAt: string; customId: string; dreamingStatus: 'dreaming' | 'done'; filepath: string; metadata: string | number | boolean | object | object[]; ogImage: string; raw: object; source: string; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; taskType: 'memory' | 'superrag'; title: string; type: string; updatedAt: string; containerTags?: string[]; memories?: object[]; url?: string; }`\n\n**get** `/v3/documents/{id}`\n\nGet a document by ID\n\n### Parameters\n\n- `id: string`\n\n### Returns\n\n- `{ id: string; connectionId: string; content: string; createdAt: string; customId: string; dreamingStatus: 'dreaming' | 'done'; filepath: string; metadata: string | number | boolean | object | object[]; ogImage: string; raw: object; source: string; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; taskType: 'memory' | 'superrag'; title: string; type: string; updatedAt: string; containerTags?: string[]; memories?: { id: string; createdAt: string; isForgotten: boolean; isInference: boolean; isLatest: boolean; isStatic: boolean; memory: string; metadata: string | number | boolean | object | object[]; parentMemoryId: string; rootMemoryId: string; sourceCount: number; updatedAt: string; version: number; }[]; url?: string; }`\n\n  - `id: string`\n  - `connectionId: string`\n  - `content: string`\n  - `createdAt: string`\n  - `customId: string`\n  - `dreamingStatus: 'dreaming' | 'done'`\n  - `filepath: string`\n  - `metadata: string | number | boolean | object | object[]`\n  - `ogImage: string`\n  - `raw: object`\n  - `source: string`\n  - `status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'`\n  - `summary: string`\n  - `taskType: 'memory' | 'superrag'`\n  - `title: string`\n  - `type: string`\n  - `updatedAt: string`\n  - `containerTags?: string[]`\n  - `memories?: { id: string; createdAt: string; isForgotten: boolean; isInference: boolean; isLatest: boolean; isStatic: boolean; memory: string; metadata: string | number | boolean | object | object[]; parentMemoryId: string; rootMemoryId: string; sourceCount: number; updatedAt: string; version: number; }[]`\n  - `url?: string`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst document = await client.documents.get('id');\n\nconsole.log(document);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: [
      "documents: { content: string; containerTag?: string; containerTags?: string[]; customId?: string; documentDate?: string; dreaming?: 'instant' | 'dynamic'; entityContext?: string; filepath?: string; filterByMetadata?: object; metadata?: object; taskType?: 'memory' | 'superrag'; }[] | string[];",
      'containerTag?: string;',
      'containerTags?: string[];',
      'content?: null;',
      'documentDate?: string;',
    ...
    response:
      '{ failed: number; results: { id: string; status: string; details?: string; error?: string; url?: string; }[]; success: number; }',
    markdown:
      "## batch_add\n\n`client.documents.batchAdd(documents: { content: string; containerTag?: string; containerTags?: string[]; customId?: string; documentDate?: string; dreaming?: 'instant' | 'dynamic'; entityContext?: string; filepath?: string; filterByMetadata?: object; metadata?: object; taskType?: 'memory' | 'superrag'; }[] | string[], containerTag?: string, containerTags?: string[], content?: null, documentDate?: string, dreaming?: 'instant' | 'dynamic', entityContext?: string, filepath?: string, filterByMetadata?: object, metadata?: object, taskType?: 'memory' | 'superrag'): { failed: number; results: object[]; success: number; }`\n\n**post** `/v3/documents/batch`\n\nAdd multiple documents in a single request. Each document can have any content type (text, url, file, etc.) and metadata\n\n### Parameters\n\n- `documents: { content: string; containerTag?: string; containerTags?: string[]; customId?: string; documentDate?: string; dreaming?: 'instant' | 'dynamic'; entityContext?: string; filepath?: string; filterByMetadata?: object; metadata?: object; taskType?: 'memory' | 'superrag'; }[] | string[]`\n\n- `containerTag?: string`\n  Optional tag this document should be containerized by. This can be an ID for your user, a project ID, or any other identifier you wish to use to group documents.\n\n- `containerTags?: string[]`\n  (DEPRECATED: Use containerTag instead) Optional tags this document should be containerized by. This can be an ID for your user, a project ID, or any other identifier you wish to use to group documents.\n\n- `content?: null`\n\n- `documentDate?: string`\n  When this document's content is from, as opposed to when it was uploaded. Accepts YYYY-MM-DD or a full ISO 8601 timestamp. Memory extraction resolves relative dates ('yesterday', 'last quarter') against this instead of the ingestion time, and documents in a batch are processed oldest-first so newer facts correctly supersede older ones. Set this whenever you backfill historical content.\n\n- `dreaming?: 'instant' | 'dynamic'`\n  Processing mode. \"dynamic\" (default) groups related documents together so memories form from coherent, logical units rather than one isolated entry at a time. \"instant\" processes each document on its own right away, and bills one extra operation per document.\n\n- `entityContext?: string`\n  Optional entity context for this container tag. Max 1500 characters. Used during document processing to guide memory extraction.\n\n- `filepath?: string`\n  Optional file path for the document (e.g., '/documents/reports/file.pdf'). Used by supermemoryfs to map documents to filesystem paths.\n\n- `filterByMetadata?: object`\n  Optional metadata filter scoping which existing memories are pulled as context during ingestion. Scalar values match exactly (AND across keys); array values match ANY (OR within key). Only memories whose source documents match this filter are used as context.\n\n- `metadata?: object`\n  Optional metadata for the document. This is used to store additional information about the document. You can use this to store any additional information you need about the document. Metadata can be filtered through. Keys must be strings and are case sensitive. Values can be strings, numbers, or booleans. You cannot nest objects.\n\n- `taskType?: 'memory' | 'superrag'`\n  Task type: \"memory\" (default) for full context layer with SuperRAG built in, \"superrag\" for managed RAG as a service.\n\n### Returns\n\n- `{ failed: number; results: { id: string; status: string; details?: string; error?: string; url?: string; }[]; success: number; }`\n\n  - `failed: number`\n  - `results: { id: string; status: string; details?: string; error?: string; url?: string; }[]`\n  - `success: number`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.documents.batchAdd({ documents: [{ content: 'Our API rate limits are 100 req/min on free and 1000 on pro. Clients should use exponential backoff on 429s.' }] });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; errorCode?: string; errorMessage?: string; stuck?: boolean; }[]; totalCount: number; pagination?: { currentPage: number; totalItems: number; totalPages: number; limit?: number; }; }",
    markdown:
      "## list_processing\n\n`client.documents.listProcessing(containerTags?: string, limit?: string | number, page?: string | number, view?: 'active' | 'pending' | 'all'): { documents: object[]; totalCount: number; pagination?: object; }`\n\n**get** `/v3/documents/processing`\n\nGet documents that are currently being processed. Default `view=active` is the live in-flight queue. `view=pending` is every unfinished document with no time cutoff. `view=all` also includes failed documents, paginated.\n\n### Parameters\n\n- `containerTags?: string`\n  Comma-separated container tags to filter by\n\n- `limit?: string | number`\n  Number of items per page. Used with `view=all`.\n\n- `page?: string | number`\n  Page number to fetch. Used with `view=all`.\n\n- `view?: 'active' | 'pending' | 'all'`\n  `active` returns in-flight documents updated in the last 4 hours. `pending` returns every document that is not done or failed, with no time cutoff. `all` also includes failed documents, paginated.\n\n### Returns\n\n- `{ documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; errorCode?: string; errorMessage?: string; stuck?: boolean; }[]; totalCount: number; pagination?: { currentPage: number; totalItems: number; totalPages: number; limit?: number; }; }`\n  List of documents currently being processed\n\n  - `documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; errorCode?: string; errorMessage?: string; stuck?: boolean; }[]`\n  - `totalCount: number`\n  - `pagination?: { currentPage: number; totalItems: number; totalPages: number; limit?: number; }`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.documents.listProcessing();\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      '{ chunkSize?: number; excludeItems?: string | number | boolean | object | object[]; filterPrompt?: string; githubClientId?: string; githubClientSecret?: string; githubCustomKeyEnabled?: boolean; googleDriveClientId?: string; googleDriveClientSecret?: string; googleDriveCustomKeyEnabled?: boolean; includeItems?: string | number | boolean | object | object[]; notionClientId?: string; notionClientSecret?: string; notionCustomKeyEnabled?: boolean; onedriveClientId?: string; onedriveClientSecret?: string; onedriveCustomKeyEnabled?: boolean; profileBuckets?: { key: string; description?: string; }[]; shouldLLMFilter?: boolean; workspacePrompt?: string; }',
    markdown:
      "## get\n\n`client.settings.get(): { chunkSize?: number; excludeItems?: string | number | boolean | object | object[]; filterPrompt?: string; githubClientId?: string; githubClientSecret?: string; githubCustomKeyEnabled?: boolean; googleDriveClientId?: string; googleDriveClientSecret?: string; googleDriveCustomKeyEnabled?: boolean; includeItems?: string | number | boolean | object | object[]; notionClientId?: string; notionClientSecret?: string; notionCustomKeyEnabled?: boolean; onedriveClientId?: string; onedriveClientSecret?: string; onedriveCustomKeyEnabled?: boolean; profileBuckets?: object[]; shouldLLMFilter?: boolean; workspacePrompt?: string; }`\n\n**get** `/v3/settings`\n\nGet settings for an organization\n\n### Returns\n\n- `{ chunkSize?: number; excludeItems?: string | number | boolean | object | object[]; filterPrompt?: string; githubClientId?: string; githubClientSecret?: string; githubCustomKeyEnabled?: boolean; googleDriveClientId?: string; googleDriveClientSecret?: string; googleDriveCustomKeyEnabled?: boolean; includeItems?: string | number | boolean | object | object[]; notionClientId?: string; notionClientSecret?: string; notionCustomKeyEnabled?: boolean; onedriveClientId?: string; onedriveClientSecret?: string; onedriveCustomKeyEnabled?: boolean; profileBuckets?: { key: string; description?: string; }[]; shouldLLMFilter?: boolean; workspacePrompt?: string; }`\n\n  - `chunkSize?: number`\n  - `excludeItems?: string | number | boolean | object | object[]`\n  - `filterPrompt?: string`\n  - `githubClientId?: string`\n  - `githubClientSecret?: string`\n  - `githubCustomKeyEnabled?: boolean`\n  - `googleDriveClientId?: string`\n  - `googleDriveClientSecret?: string`\n  - `googleDriveCustomKeyEnabled?: boolean`\n  - `includeItems?: string | number | boolean | object | object[]`\n  - `notionClientId?: string`\n  - `notionClientSecret?: string`\n  - `notionCustomKeyEnabled?: boolean`\n  - `onedriveClientId?: string`\n  - `onedriveClientSecret?: string`\n  - `onedriveCustomKeyEnabled?: boolean`\n  - `profileBuckets?: { key: string; description?: string; }[]`\n  - `shouldLLMFilter?: boolean`\n  - `workspacePrompt?: string`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst setting = await client.settings.get();\n\nconsole.log(setting);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: [
      '{ workspacePrompt: string; } | { chunkSize?: number; excludeItems?: string | number | boolean | object | object[]; filterPrompt?: string; githubClientId?: string; githubClientSecret?: string; githubCustomKeyEnabled?: boolean; googleDriveClientId?: string; googleDriveClientSecret?: string; googleDriveCustomKeyEnabled?: boolean; includeItems?: string | number | boolean | object | object[]; notionClientId?: string; notionClientSecret?: string; notionCustomKeyEnabled?: boolean; onedriveClientId?: string; onedriveClientSecret?: string; onedriveCustomKeyEnabled?: boolean; profileBuckets?: { key: string; description?: string; }[]; shouldLLMFilter?: boolean; };',
    ],
    response:
    ...
    perLanguage: {
      typescript: {
        ...
        example:
          "import Supermemory from 'supermemory';\n\nconst client = new Supermemory({\n  apiKey: process.env['SUPERMEMORY_API_KEY'], // This is the default and can be omitted\n});\n\nconst setting = await client.settings.update({ workspacePrompt: 'workspacePrompt' });\n\nconsole.log(setting.orgId);",
      },
      python: {
  ...
  },
  {
    ...
    response:
      "{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }",
    markdown:
      "## get_by_id\n\n`client.connections.getByID(connectionId: string): { id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: object; metadata?: object; }`\n\n**get** `/v3/connections/{connectionId}`\n\nGet connection details with id\n\n### Parameters\n\n- `connectionId: string`\n\n### Returns\n\n- `{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }`\n\n  - `id: string`\n  - `createdAt: string`\n  - `provider: string`\n  - `containerTags?: string[]`\n  - `documentLimit?: number`\n  - `email?: string`\n  - `expiresAt?: string`\n  - `lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }`\n  - `metadata?: object`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.connections.getByID('connectionId');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }",
    markdown:
      "## get_by_tag\n\n`client.connections.getByTag(provider: 'notion' | 'google-drive' | 'onedrive' | 'gmail' | 'github' | 'web-crawler' | 's3' | 'granola', containerTags: string[]): { id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: object; metadata?: object; }`\n\n**post** `/v3/connections/{provider}/connection`\n\nGet connection details with provider and container tags\n\n### Parameters\n\n- `provider: 'notion' | 'google-drive' | 'onedrive' | 'gmail' | 'github' | 'web-crawler' | 's3' | 'granola'`\n\n- `containerTags: string[]`\n  Comma-separated list of container tags to filter connection by\n\n### Returns\n\n- `{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }`\n\n  - `id: string`\n  - `createdAt: string`\n  - `provider: string`\n  - `containerTags?: string[]`\n  - `documentLimit?: number`\n  - `email?: string`\n  - `expiresAt?: string`\n  - `lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }`\n  - `metadata?: object`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.connections.getByTag('notion', { containerTags: ['user_123', 'project_123'] });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }[]",
    markdown:
      "## list\n\n`client.connections.list(containerTags?: string[]): { id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: object; metadata?: object; }[]`\n\n**post** `/v3/connections/list`\n\nList all connections\n\n### Parameters\n\n- `containerTags?: string[]`\n  Optional comma-separated list of container tags to filter documents by\n\n### Returns\n\n- `{ id: string; createdAt: string; provider: string; containerTags?: string[]; documentLimit?: number; email?: string; expiresAt?: string; lastSyncRun?: { status: string; completedAt?: string; error?: string; errorCode?: 'auth_expired' | 'rate_limited' | 'plan_required' | 'provider_unavailable' | 'internal'; startedAt?: string; }; metadata?: object; }[]`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst connections = await client.connections.list();\n\nconsole.log(connections);\n```",
    perLanguage: {
      typescript: {
