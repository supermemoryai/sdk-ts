...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['id: string;'],
    response:
      "{ id: string; connectionId: string; content: string; createdAt: string; customId: string; filepath: string; metadata: string | number | boolean | object | object[]; ogImage: string; raw: object; source: string; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; taskType: 'memory' | 'superrag'; title: string; type: string; updatedAt: string; containerTags?: string[]; url?: string; }",
    markdown:
