...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response: '{ id: string; status: string; }',
    markdown:
      "## add\n\n`client.documents.add(content: string, containerTag?: string, containerTags?: string[], customId?: string, dreaming?: 'instant' | 'dynamic', entityContext?: string, filepath?: string, filterByMetadata?: object, metadata?: object, taskType?: 'memory' | 'superrag'): { id: string; status: string; }`\n\n**post** `/v3/documents`\n\nAdd a document with any content type (text, url, file, etc.) and metadata\n\n### Parameters\n\n- `content: string`\n  The content to extract and process into a document. This can be a URL to a website, a PDF, an image, or a video.\n\n- `containerTag?: string`\n  Optional tag this document should be containerized by. Max 100 characters, alphanumeric with hyphens, underscores, and dots only.\n\n- `containerTags?: string[]`\n\n- `customId?: string`\n  Optional custom ID of the document. Max 100 characters, alphanumeric with hyphens, underscores, and dots only.\n\n- `dreaming?: 'instant' | 'dynamic'`\n  Processing mode. \"dynamic\" (default) groups related documents together so memories form from coherent, logical units rather than one isolated entry at a time. \"instant\" processes each document on its own right away, and bills one extra operation per document.\n\n- `entityContext?: string`\n  Optional entity context for this container tag. Max 1500 characters. Used during document processing to guide memory extraction.\n\n- `filepath?: string`\n  Optional file path for the document. Used by supermemoryfs to store the full path of the file.\n\n- `filterByMetadata?: object`\n  Optional metadata filter to apply when pulling related memories and profile during ingestion. Only memories matching these filters will be used as context.\n\n- `metadata?: object`\n  Optional metadata for the document.\n\n- `taskType?: 'memory' | 'superrag'`\n  Task type: \"memory\" (default) for full context layer with SuperRAG built in, \"superrag\" for managed RAG as a service.\n\n### Returns\n\n- `{ id: string; status: string; }`\n\n  - `id: string`\n  - `status: string`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.documents.add({ content: 'content' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    params: [
      "documents: { content: string; containerTag?: string; containerTags?: string[]; customId?: string; dreaming?: 'instant' | 'dynamic'; entityContext?: string; filepath?: string; filterByMetadata?: object; metadata?: object; taskType?: 'memory' | 'superrag'; }[] | string[];",
      'containerTag?: string;',
      'containerTags?: string[];',
      'content?: null;',
      "dreaming?: 'instant' | 'dynamic';",
  ...
  },
  {
    ...
    response:
      "{ documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; }[]; totalCount: number; }",
    markdown:
      "## list_processing\n\n`client.documents.listProcessing(): { documents: object[]; totalCount: number; }`\n\n**get** `/v3/documents/processing`\n\nGet documents that are currently being processed\n\n### Returns\n\n- `{ documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; }[]; totalCount: number; }`\n  List of documents currently being processed\n\n  - `documents: { id: string; connectionId: string; createdAt: string; customId: string; metadata: string | number | boolean | object | object[]; status: 'unknown' | 'queued' | 'extracting' | 'chunking' | 'embedding' | 'indexing' | 'done' | 'failed'; summary: string; title: string; type: string; updatedAt: string; containerTags?: string[]; }[]`\n  - `totalCount: number`\n\n### Example\n\n```typescript\nimport Supermemory from 'supermemory';\n\nconst client = new Supermemory();\n\nconst response = await client.documents.listProcessing();\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
